...
import { ImportedListsPanel } from './imported-lists-panel';
import {
...
type ContactTab = 'todos' | 'pessoas' | 'grupos' | 'importacoes';
type SortField = 'name' | 'phone' | 'company' | 'lastContactAt' | 'ownerName';
type SortOrder = 'asc' | 'desc';

interface ContactsExplorerProps {
  readonly contacts: readonly Contact[];
  readonly importBatches: readonly ContactImportBatchSummary[];
  /**
...
export function ContactsExplorer({ contacts, importBatches, canExport }: ContactsExplorerProps) {
  ...
  const tabCounts = useMemo(
    () => ({
      ...
      importacoes: importBatches.length,
    }),
    [contacts, importBatches],
  );
    ...
    <>
      <div className="flex flex-col gap-4 animate-in fade-in duration-150">
        ...
        <div className="flex items-center gap-1.5 border-b border-line pb-2">
          ...
          >
            ...
            <span
              className={cn(
                ...
                tab === 'todos'
                  ? 'bg-brand text-white'
                  : 'bg-surface-2 text-dim border border-line-soft',
              )}
            >
              {tabCounts.todos}
            </span>
          ...
          <button
            type="button"
            onClick={() => trocarTab('importacoes')}
            className={cn(
              'inline-flex items-center gap-2 rounded-lg px-3.5 py-1.5 text-xs font-semibold transition-all',
              tab === 'importacoes'
                ? 'border border-violet-500/30 bg-violet-500/15 text-violet-600 shadow-xs dark:text-violet-400'
            ...
            <span
              className={cn(
                ...
                tab === 'importacoes'
                  ...
                  : 'border border-line-soft bg-surface-2 text-dim',
              )}
            >
              {tabCounts.importacoes}
            </span>
          </button>
        </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-ink">
                      Gestão e Autorização de Grupos no Chat
                    </h4>
                    <p className="text-xs text-muted mt-0.5">
                      Selecione quais grupos têm permissão para interagir e gravar mensagens no chat
                      do CRM. Grupos desativados são ignorados e não poluem o banco nem a caixa de
                      entrada.
                    </p>
                  </div>
                </div>
            ...
            {contacts.length === 0 ? (
              ...
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-line bg-surface p-12 text-center shadow-xs">
                ...
                <p className="mt-1.5 max-w-sm text-body text-muted leading-relaxed">
                  Comece cadastrando novos clientes manualmente ou faça uma importação rápida de
                  planilha CSV.
                </p>
                  ...
                  <table className="w-full min-w-[960px] text-left text-body border-collapse">
                    ...
                    <thead className="border-b border-line bg-surface-2/70 text-meta text-dim uppercase tracking-wider select-none font-semibold">
                      <tr>
                        ...
                        >
                          <div className="flex items-center gap-1.5">
                            <span>
                              {tab === 'grupos' ? 'Atendimento no Chat' : 'Empresa / Status'}
                            </span>
                            {sortField === 'company' ? (
                              sortOrder === 'asc' ? (
                                <ArrowUp className="size-3.5 text-brand" />
                    ...
                    <tbody className="divide-y divide-line-soft">
                      {pagedContacts.map((contact) => {
                        ...
                        const nameIsPhone =
                          !isGroup &&
                          PhoneNumber.normalize(contact.name) ===
                            PhoneNumber.normalize(contact.phone);
                        ...
                        return (
                          <tr
                            ...
                            className={cn(
                              ...
                              isChecked ? 'bg-brand/5 dark:bg-brand/10' : 'hover:bg-surface-2/60',
                            )}
                          >
                            {/* Checkbox */}
                              ...
                              {isGroup ? (
                                <div className="flex items-center gap-2">
                                  <button
                                    ...
                                    title={
                                      isAllowed
                                        ? 'Desativar grupo do chat'
                                        : 'Autorizar grupo no chat'
                                    }
                                    className={cn(
                                      ...
                                      isAllowed
                                        ? 'bg-emerald-600'
                                        : 'bg-line-strong dark:bg-surface-2',
                ...
                <div className="flex flex-col gap-3 border-t border-line px-5 py-3 text-meta text-muted bg-surface-2/30 sm:flex-row sm:items-center sm:justify-between">
                  <span>
                    ...
                    <strong className="text-ink font-semibold">
                      {pagedContacts.length === 0 ? 0 : pageStart + 1}–
                      {pageStart + pagedContacts.length}
                    </strong>{' '}
      ...
      <NewContactModal open={isNewContactOpen} onClose={() => setIsNewContactOpen(false)} />
      ...
      <ImportCsvModal open={isImportOpen} onClose={() => setIsImportOpen(false)} />
